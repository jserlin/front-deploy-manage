...
class SVNService {
  ...
  async ensureSvnDirectoryExists(credential, fullSvnUrl) {
    const password = credential.password ? CryptoUtil.decrypt(credential.password) : "";
    try {
    ...
    try {
      await this.execCommand(mkdirCommand, { timeout: 3e4 });
  ...
  async uploadDirectory(credential, localPath, svnPath, commitMessage) {
    ...
    try {
      ...
      await lib.remove(tempDir).catch(() => {
      });
      ...
      await this.execCommand(checkoutCommand, { timeout: 6e4, maxBuffer: 10 * 1024 * 1024 });
      this.checkAborted();
      await lib.copy(localPath, tempDir, { overwrite: true });
      const addCommand = `svn add --force "${tempDir}" --auto-props --parents --depth infinity -q`;
      await this.execCommand(addCommand, { timeout: 6e4, maxBuffer: 10 * 1024 * 1024 });
      const commitCommand = `svn commit --username "${credential.username}" --password "${password}" --non-interactive -m "${commitMessage}" "${tempDir}" -q`;
      await this.execCommand(commitCommand, { timeout: 3e5, maxBuffer: 10 * 1024 * 1024 });
  ...
  async uploadFile(credential, localFilePath, svnPath, targetFileName, commitMessage) {
    ...
    try {
      ...
      await this.execCommand(checkoutCommand, { timeout: 6e4, maxBuffer: 10 * 1024 * 1024 });
      this.checkAborted();
      const destPath = path__namespace.join(tempDir, targetFileName);
      await lib.copy(localFilePath, destPath, { overwrite: true });
      const addCommand = `svn add --force "${tempDir}" --auto-props --parents --depth infinity -q`;
      await this.execCommand(addCommand, { timeout: 6e4, maxBuffer: 10 * 1024 * 1024 });
      const commitCommand = `svn commit --username "${credential.username}" --password "${password}" --non-interactive -m "${commitMessage}" "${tempDir}" -q`;
      await this.execCommand(commitCommand, { timeout: 3e5, maxBuffer: 10 * 1024 * 1024 });
...
class BuildService {
  ...
  getNodeEnvPath(nodeVersion) {
    ...
    const nodeDir = path__namespace.join(nvmHome, versionPrefix);
...
class NodeVersionService {
  ...
  normalizeVersion(version) {
    const trimmed2 = version.trim();
    return trimmed2.startsWith("v") ? trimmed2 : `v${trimmed2}`;
  }
...
function registerIpcHandlers(database2) {
  ...
  require$$0$6.ipcMain.handle("project:create", async (event, data) => {
    try {
      ...
      `, [
        ...
        data.nodeVersion ? data.nodeVersion.trim() : null,
        data.permissionFilePath || null,
        data.svnPermissionAlias || null,
  ...
  require$$0$6.ipcMain.handle("project:scanSubProjects", async (event, repoPath) => {
    try {
      if (!repoPath || !lib.existsSync(repoPath)) {
        return { success: false, error: "仓库路径不存在" };
      }
      const skipDirs = /* @__PURE__ */ new Set(["node_modules", ".git", "dist", "build", "target", ".svn", "doc", "docs", ".idea", ".vscode", "static", "public"]);
      const results = [];
      const scanDir = async (dir, depth) => {
        if (depth > 2) return;
        let entries = [];
        try {
          entries = await lib.readdir(dir);
        } catch {
          return;
        }
        for (const entry of entries) {
          ...
          let stat2;
          try {
            stat2 = await lib.stat(fullPath);
          } catch {
            continue;
          }
          if (!stat2.isDirectory()) continue;
          const pkgPath = path__namespace.join(fullPath, "package.json");
          if (lib.existsSync(pkgPath)) {
            try {
              const pkg = await lib.readJson(pkgPath);
              const scripts = pkg.scripts || {};
              const hasBuild = !!(scripts.build || scripts["build:prod"] || scripts["build:dev"]);
              if (hasBuild) {
                const buildCommand = scripts.build ? "npm run build" : scripts["build:prod"] ? "npm run build:prod" : "npm run build:dev";
                let outputDir = "dist";
                try {
                  const cfgFiles = await lib.readdir(fullPath);
                ...
                results.push({
                  name: pkg.name || entry,
                  localPath: fullPath,
                  relativePath: path__namespace.relative(repoPath, fullPath),
                  buildCommand,
                  outputDir,
                  nodeVersion: ""
                });
  ...
  require$$0$6.ipcMain.handle("project:detectRepoRoot", async (event, localPath) => {
    try {
  ...
  require$$0$6.ipcMain.handle("deploy:svn", async (event, config) => {
    ...
    try {
      ...
      if (syncPermissionFile && project.permissionFilePath) {
        const srcFile = path__namespace.basename(project.permissionFilePath);
        const targetName = project.svnPermissionAlias || srcFile;
        const permSvnPath = permissionSvnPath || svnPath;
        log2(`同步权限文件: ${srcFile}${targetName !== srcFile ? " → " + targetName : ""}`);
  ...
  require$$0$6.ipcMain.handle("deploy:mixed", async (event, config) => {
    ...
    try {
      ...
      if (syncPermissionFile && project.permissionFilePath) {
        const svnTarget = targets.find((t2) => t2.type === "svn");
        if (svnTarget) {
          const srcFile = path__namespace.basename(project.permissionFilePath);
          const targetName = project.svnPermissionAlias || srcFile;
  ...
  require$$0$6.ipcMain.handle("deploy:batch", async (event, config) => {
    sshService.resetAbort();
    svnService.resetAbort();
    const logs = [];
    const log2 = (msg) => {
    ...
    try {
      ...
      for (const [repoKey, groupProjects] of repoGroups) {
        ...
        for (const project of groupProjects) {
          const subLogs = [];
          log2(`===== 开始处理项目: ${project.name} =====`);
          try {
            if (needBuild) {
              log2(`[${project.name}] 开始构建...`);
              const buildResult = await buildService.build(project, (l) => {
                subLogs.push(l);
              ...
              const isValid = await buildService.validateOutput(project);
              if (!isValid) throw new Error("构建产物验证失败");
              log2(`[${project.name}] 构建完成`);
            } else {
              log2(`[${project.name}] 跳过构建，使用已有产物`);
            }
            const outputPath = path__namespace.join(project.localPath, project.outputDir);
            for (const target of targets) {
              if (target.type === "svn") {
                log2(`[${project.name}] 上传到 SVN: ${target.svnPath}`);
                await svnService.uploadDirectory(target.credential, outputPath, target.svnPath, target.commitMessage);
                log2(`[${project.name}] SVN 上传完成`);
              } else if (target.type === "server") {
                log2(`[${project.name}] 压缩并上传到服务器: ${target.credential.host}:${target.remotePath}`);
                await sshService.uploadDirectoryCompressed(target.credential, outputPath, target.remotePath, (progress) => {
                  event.sender.send("deploy:progress", { stage: "uploading", progress });
                });
                log2(`[${project.name}] 服务器上传完成`);
              }
            }
            const now = (/* @__PURE__ */ new Date()).toLocaleString("sv-SE");
            db.run(
            ...
            log2(`[${project.name}] 发布失败: ${err.message}`);
          }
        }
      }
      const allSuccess = results.every((r2) => r2.success);
      event.sender.send("deploy:progress", {
  ...
  require$$0$6.ipcMain.handle("template:getAll", async () => {
    try {
      ...
      const mapped = rows.map((t2) => ({
        ...
        permissionSvnPath: t2.permission_svn_path || "",
  ...
  require$$0$6.ipcMain.handle("config:import", async (event, filePath) => {
    try {
      ...
      for (const project of config.projects || []) {
        try {
          db.run(
            "INSERT OR REPLACE INTO projects (id, name, local_path, git_repo, git_branch, build_command, output_dir, group_id, description, permission_file_path, svn_permission_alias, repo_root_path) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
            [project.id, project.name, project.local_path, project.git_repo, project.git_branch, project.build_command, project.output_dir, project.group_id, project.description, project.permission_file_path, project.svn_permission_alias, project.repo_root_path]
          );
        } catch (e) {
        }
      }
  ...
  require$$0$6.ipcMain.handle("config:selectSingleFile", async () => {
    try {
      const result = await require$$0$6.dialog.showOpenDialog({
        title: "选择权限文件",
        properties: ["openFile"],
  ...
  require$$0$6.ipcMain.handle("config:readDoc", async (_event, fileName) => {
    try {
      ...
      const app2 = await import("electron").then((m) => m.app);
      const isPackaged = app2.isPackaged;
      const docBase = isPackaged ? path2.join(process.resourcesPath, "doc") : path2.join(process.cwd(), "doc");
      const docPath = path2.join(docBase, fileName);
