...
const submitForm = async () => {
  try {
    ...
    if (isEdit.value && editingId.value) {
      result = await window.electronAPI.svnCredential.update(editingId.value, formData.value)
    } else {
      result = await window.electronAPI.svnCredential.create(formData.value)
    }
