    ...
    <el-table :data="filteredCredentials" v-loading="loading" border stripe>
      ...
      <el-table-column prop="authType" label="认证方式" width="100">
        <template #default="{ row }">
          <el-tag :type="row.authType === 'password' ? 'success' : 'warning'">
...
import type { ServerCredential } from '@/types/credential'

const loading = ref(false)
const credentials = ref<ServerCredential[]>([])
...
const fetchCredentials = async () => {
  loading.value = true
  try {
    const result = await window.electronAPI.serverCredential.getAll()
    if (result.success) {
...
const editCredential = (credential: ServerCredential) => {
  isEdit.value = true
...
const deleteCredential = async (credential: ServerCredential) => {
  try {
    ...
    const result = await window.electronAPI.serverCredential.delete(credential.id)
    if (result.success) {
...
const testConnection = async (credential: any) => {
  ...
  try {
    const result = await window.electronAPI.serverCredential.testConnection(credential.id)
    if (result.success) {
