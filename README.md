...
- **混合发布**: 单次构建，同时发布到多个目标
- **批量发布**: 整合仓库内多个子前端项目一次性发布（Git 操作只执行一次）
- **压缩上传**: 使用 tar 压缩后上传，大幅提升传输速度
- **分支容错**: 本地存在但远程不存在的分支不中断发布流程，记录详细日志
- 发布进度实时显示
- 支持取消发布
- 发布日志记录和查看（含完整 commit 信息和 tag 标签）
...
2. 点击「扫描整合仓库」按钮
3. 选择整合仓库根目录，点击「扫描」
4. 系统自动识别仓库内所有具备构建能力的前端项目
5. 勾选需要导入的子项目，选择分组
6. 点击「导入选中 N 项」完成批量导入

> 整合仓库模式下，Git 操作在仓库根目录执行，各子项目独立构建，互不干扰。
...
### 3.1 批量发布（整合仓库）
1. 选择一个整合仓库项目后，页面会显示「同仓库项目」多选框
2. 勾选需要一同发布的兄弟项目
3. 配置发布目标后点击「批量发布 (N)」
4. 系统自动：仓库 pull/checkout 一次 → 各子项目独立构建 → 各自上传
